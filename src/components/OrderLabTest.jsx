...
const OrderLabTest = ({ show, onHide, patientName }) => {
  ...
  const handleTestSelect = (event, value) => {
    if (value) {
      setSelectedTests((prevTests) => [...prevTests, { name: value, quantity: 1 }]);
    }
  };
  ...
  const handleQuantityDecrease = (index) => {
    setSelectedTests((prevTests) => {
      const updatedTests = [...prevTests];
      if (updatedTests[index].quantity > 1) {
        updatedTests[index].quantity -= 1;
      }
      return updatedTests;
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!specimenType) {
      toast.error('Please enter the specimen type.', {});
      return;
    }
